  ...
  return (
    ...
    >
      ...
      <div className='flex flex-col items-start justify-start h-[120px]'>
          ...
          {marcas &&  (
              <div className="flex justify-start items-center mt-0 md:mt-1 h-6 lg:h-7">  
                <img
                  ...
                  className="h-6 lg:h-7 w-auto" // Ajusta el tamaño como necesites
        ...
        <a href={`/producto/${item.id}`} className='p-0'>
              
          <Tippy content={item.producto}>
            <h2 className="block text-sm xl:text-base text-left overflow-hidden font-medium text-[#808080] font-Urbanist_Regular" style={{ display: '-webkit-box', WebkitLineClamp: 1, textOverflow: 'ellipsis', WebkitBoxOrient: 'vertical', height: '31' }}>
              {item.producto}
            </h2>
          </Tippy>
        ...
        
          {
            is_reseller ?
              (<>
                ...
                <div className="flex content-between flex-row gap-4 items-center justify-start">
                  Reseller <span className="text-[#c1272d] text-[16.45px] font-bold">S/. {item.precio_reseller}</span>

                </div></>

              ) :
              (<div className="w-full flex content-between flex-row gap-4 items-center justify-start text-left font-Urbanist_Light">
                <span className="text-[#c1272d] font-Urbanist_Regular font-bold">{item.descuento > 0 ? 
                  
                <div className='flex flex-col justify-start items-center'>
                  <span className='text-base lg:text-lg'>S/. {item.descuento}</span>
                </div> : 

                <div className='flex flex-col justify-start items-center text-lg '>
                  <span className='font-bold text-base lg:text-lg'>S/. {item.precio}</span>
                </div>
                ...
                {item.descuento > 0 && (
                  <>
                    <div className='flex flex-col items-start'>
                      <span className=" text-[#808080] opacity-80 line-through font-Urbanist_Regular text-[12px] md:text-sm"> S/. {item.precio}</span>
                    </div>
                  </>
                )}
              </div>)
