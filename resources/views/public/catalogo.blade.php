...
@section('content')
  ...
  @endcomponent --}}
            ...
            <div class="w-full relative flex justify-center items-center">
                <div class="bg-[#F0F1F0]  justify-center items-center max-w-5xl gap-10 rounded-3xl p-6">
                    <div>
                                    ...
                                    <div class="swiper-slide">
                                        <div class="flex flex-col justify-center items-center">
                                            <a href="{{ route('catalogo', $categorias->id) }}"
                                                class="flex flex-row items-center min-h-[75px]">
                                                <h2
                                                    class="@if ($id_cat == $categorias->id) activo @endif  underline-this leading-none font-akira_expanded max-w-[300px] text-center text-[26px] text-[#010101]">
                                                    {{ $categorias->name }}
                                                </h2>
                                            </a>
                                        </div>
                        ...
                        <div
                            class="hidden md:flex swiper-carrusel_cat-prev absolute top-1/2 -translate-y-1/2 -left-2 lg:-left-5 z-20 bg-white rounded-full">
                            <i class="fa-solid fa-circle-chevron-left text-5xl text-[#FB4535]"></i>
                        </div>
                        <div
                            class="hidden md:flex swiper-carrusel_cat-next absolute top-1/2 -translate-y-1/2 -right-2 lg:-right-5 z-20 bg-white rounded-full">
                            <i class="fa-solid fa-circle-chevron-right text-5xl text-[#FB4535]"></i>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <section class="flex flex-col gap-10 w-full px-[5%] xl:px-[8%] py-10 md:py-20">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 md:gap-16 w-full ">
                ...
                <div class="flex flex-col gap-3 w-full">
                    ...
                    @php
                        ...
                        if (!empty($categoria->duracion)) {
                            ...
                            $duraciones = array_filter(
                                array_map(fn($text) => trim(strip_tags($text)), $matches[1]),
                                fn($text) => !empty($text),
                            );
                        }
                    @endphp
                    ...
                    @if (!empty($duraciones))
                        <div class="flex flex-row gap-1">
                            <svg class="w-10" xmlns="http://www.w3.org/2000/svg" width="14" height="26"
                                viewBox="0 0 14 26" fill="none">
                                <path
                                    d="M1.15499 18.225L6.87898 25.5287L12.6709 18.1401C11.7028 18.7686 10.225 19.2951 7.93206 19.4989L13.724 14.5393L12.7728 12.5011L8.95116 4.39915L6.87898 0L4.8068 4.41614L0.96815 12.518L0 14.5732L5.77494 19.4989C3.58386 19.2951 2.12314 18.8195 1.15499 18.225Z"
                                    fill="url(#paint0_linear_52_2722)" />
                                <defs>
                                    <linearGradient id="paint0_linear_52_2722" x1="-16.8693" y1="89.3844" x2="-16.8693"
                                        y2="6.06777" gradientUnits="userSpaceOnUse">
            ...
            <div class="flex flex-col justify-start gap-5">
                <img class="object-cover aspect-square galeriatotal rounded-3xl max-h-[500px] w-full cursor-pointer"
                    src="{{ asset($categoria->url_image . $categoria->name_image) }}"
                    onerror="this.onerror=null;this.src='{{ asset('images/imagen/imagencatalog.png') }}';" />
                <div class="grid grid-cols-2 gap-5">
                    @foreach ($galeria as $galery)
                        <img class="object-cover galeriatotal aspect-square rounded-2xl max-h-[500px] w-full cursor-pointer"
                            src="{{ asset($galery->imagen) }}"
                            onerror="this.onerror=null;this.src='{{ asset('images/imagen/imagengrid.png') }}';" />
                    @endforeach
                </div>
            </div>
    ...
    @if (count($productos) > 0)
        <section id="planes" class="flex flex-col gap-10 w-full px-[5%] xl:px-[8%] py-10 md:py-14 bg-[#F0F1F0]">
            ...
            <div class="w-full relative">
                            ...
                            <div class="swiper-slide">
                                <div
                                    ...
                                    <div class="flex flex-col gap-2">
                                        <h2
                                            class="leading-none font-akira_expanded text-2xl xl:text-3xl 
                                        ...
                                        <div
                                            class="text-base font-roboto_regular
                                                {{ $producto->sku == 1 ? '!text-[#010101]' : '' }}
                                                {{ $producto->sku == 2 ? '!text-white' : '' }}
                                                {{ $producto->sku == 3 ? '!text-white' : '' }}
                                                {{ !in_array($producto->sku, [1, 2, 3]) ? '!text-white' : '' }}
                                    ...
                                    rounded-3xl p-3 flex flex-row items-center justify-center">
                                            <span class="text-white font-roboto_bold text-center">Contratar
                                    ...
                                    @if (!empty($caracteristicas))
                                        <div class="flex flex-col">
                                            @foreach ($caracteristicas as $index => $caracteristica)
                                                ...
                                                {{ $producto->sku == 3 ? '!text-white' : '' }}
                                                {{ !in_array($producto->sku, [1, 2, 3]) ? '!text-white' : '' }}
                                                 text-base font-roboto_medium w-full">
                                                        {!! $caracteristica !!}</h2>
                                                </div>
                ...
                <div
                    class="swiper-carrusel_planes-prev overflow-hidden absolute top-1/2 -translate-y-1/2 -left-2 lg:-left-12 z-20 bg-white rounded-full">
                    <i class="fa-solid fa-circle-chevron-left text-3xl md:text-5xl text-[#000000]"></i>
                </div>
                <div
                    class="swiper-carrusel_planes-next overflow-hidden absolute top-1/2 -translate-y-1/2 -right-2 lg:-right-12 z-20 bg-white rounded-full">
                    <i class="fa-solid fa-circle-chevron-right  text-3xl md:text-5xl text-[#000000]"></i>
                </div>
            </div>
    ...
    <div id="modalgaleriatotal" class="modal !px-[0px] !py-[0px] !z-50"
            ...
            <div class="">
                <div class="swiper galeriadeimagenes">
                    <div class="swiper-wrapper">
                        ...
                        @foreach ($galeria as $galery)
                            <div class="swiper-slide">
                                <img loading="lazy" src="{{ asset($galery->imagen) }}"
                                    class="object-cover w-full max-h-[450px] rounded-none overflow-hidden" />
                            </div>
                        @endforeach

                        @if ($categoria->name_image)
                            <div class="swiper-slide">
                                <img loading="lazy" src="{{ asset($categoria->url_image . $categoria->name_image) }}"
                                    class="object-cover w-full max-h-[450px] rounded-none overflow-hidden" />
                            </div>
                        @endif
    ...
    <div class="flex flex-col gap-3">
        <template x-if="planes.length > 0">
            <button @click="pagarCompleto()"
                ...
                :class="{ 'opacity-50 cursor-not-allowed': !isValidEmail }" :disabled="!isValidEmail">
...
<script>
    ...
    // Modificar la función existente de openPlanesModal
    function openPlanesModal(id, producto, precio) {
        const button = document.querySelector(`button[data-id="${id}"]`);
        const planes = JSON.parse(button.dataset.planes || '[]');
        const percent_discount = Number(button.dataset.percentdiscount) || 0;
